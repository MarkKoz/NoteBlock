...
public class Pitch {
	...
	/**
	 * Creates a Pitch object from a pitch value, returning null if the pitch
	 * isn't supported
	 *
	...
	public NoteOctavePair getNoteOctavePair() {
		return new NoteOctavePair(Note.fromPitch(this), Octave.fromPitch
				(this));
	}
	...
	public Pitch incrementOctave(int factor) {
		return Pitch.fromNoteOctave(this.getNote(), this.getOctave().add
				(factor));
	}
